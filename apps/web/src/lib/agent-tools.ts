...
const AGENT_TOOLS_RAW: AgentTool[] = [
  ...
  {
    id: "b20-deploy-guide",
    name: "B20 Deploy Guide",
    description: "Step-by-step B20 token deployment guide — variant selection, role setup, PolicyRegistry, minting, verification.",
    agentHandle: "composite", agentName: "Blue Agent", agentType: "composite",
    category: "security",
    inputs: [
      { key: "step", label: "Step", placeholder: "full / variant / factory / roles / policy / mint / verify / checklist", required: false },
      { key: "variant", label: "Variant", placeholder: "Asset or Stablecoin", required: false },
      { key: "context", label: "Context (optional)", placeholder: "Token type, use case", required: false },
    ],
    isComposite: true,
    price: "$0.10", priceUSDC: 100000,
    x402Url: `${X402_BASE}/b20-deploy-guide`,
    x402Body: (v) => ({ step: v.step ?? "full", variant: v.variant, context: v.context }),
  },
  {
    id: "liquidity-depth",
