...
export async function POST(req: NextRequest) {
  ...
  console.log("[token-launch-readiness] Bankr 502 → falling back to local handler");
  try {
    let body: Record<string, unknown> = {};
    try { body = await cloned.json(); } catch {}
    return await handleLocally(body);
  } catch (error) {
    console.error("[token-launch-readiness] Local handler failed:", error);
    return NextResponse.json(
      { error: "Token launch readiness check failed", message: (error as Error).message },
      { status: 500 }
    );
  }
}
