...
export async function POST(req: NextRequest) {
  ...
  console.log("[stack-recommender] Bankr 502 → falling back to local handler");
  try {
    let body: Record<string, unknown> = {};
    try { body = await cloned.json(); } catch {}
    return await handleLocally(body);
  } catch (error) {
    console.error("[stack-recommender] Local handler failed:", error);
    return NextResponse.json(
      { error: "Stack recommender failed", message: (error as Error).message },
      { status: 500 }
    );
  }
}
