...
export async function POST(req: NextRequest) {
  ...
  console.log("[agent-collab-match] Bankr 502 → falling back to local handler");
  try {
    let body: Record<string, unknown> = {};
    try { body = await cloned.json(); } catch {}
    return await handleLocally(body);
  } catch (error) {
    console.error("[agent-collab-match] Local handler failed:", error);
    return NextResponse.json(
      { error: "Agent collab match failed", message: (error as Error).message },
      { status: 500 }
    );
  }
}
