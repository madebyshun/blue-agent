...
export async function POST(req: NextRequest) {
  ...
  console.log("[community-growth-playbook] Bankr 502 → falling back to local handler");
  try {
    let body: Record<string, unknown> = {};
    try { body = await cloned.json(); } catch {}
    return await handleLocally(body);
  } catch (error) {
    console.error("[community-growth-playbook] Local handler failed:", error);
    return NextResponse.json(
      { error: "Community growth playbook failed", message: (error as Error).message },
      { status: 500 }
    );
  }
}
