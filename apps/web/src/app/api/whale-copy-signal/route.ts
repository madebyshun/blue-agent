...
export async function POST(req: NextRequest) {
  ...
  console.log("[whale-copy-signal] Bankr 502 → falling back to local handler");
  try {
    let body: Record<string, unknown> = {};
    try { body = await cloned.json(); } catch {}
    return await handleLocally(body);
  } catch (error) {
    console.error("[whale-copy-signal] Local handler failed:", error);
    return NextResponse.json(
      { error: "Whale copy signal failed", message: (error as Error).message },
      { status: 500 }
    );
  }
}
