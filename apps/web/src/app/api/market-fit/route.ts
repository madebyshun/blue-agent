...
export async function POST(req: NextRequest) {
  ...
  console.log("[market-fit] Bankr 502 → falling back to local handler");
  try {
    let body: Record<string, unknown> = {};
    try { body = await cloned.json(); } catch {}
    return await handleLocally(body);
  } catch (error) {
    console.error("[market-fit] Local handler failed:", error);
    return NextResponse.json(
      { error: "Market fit validation failed", message: (error as Error).message },
      { status: 500 }
    );
  }
}
