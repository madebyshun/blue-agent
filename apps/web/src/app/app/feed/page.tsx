...
function FeedCard({ item, copied, onShare }: { item: FeedItem; copied: boolean; onShare: () => void }) {
  ...
  const metrics = (Array.isArray((item.data as { metrics?: Metric[] })?.metrics)
    ...
    : []) as Metric[];
  return (
    <div className="ba-card rounded-2xl p-5 h-full flex flex-col">
      ...
      <p className="text-[13px] text-slate-400 leading-relaxed mb-4 flex-1">{item.summary}</p>
...
export default function FeedPage() {
  ...
  const [updatedAt, setUpdated] = useState(0);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setUpdated(typeof data.updatedAt === "number" ? data.updatedAt : 0);
    } catch {
      setItems([]);
    } finally {
      setLoading(false);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-3 px-4 sm:px-6 py-4 border-b border-[#1A1A2E] shrink-0">
        <div className="min-w-0">
          ...
          <p className="font-mono text-[11px] text-slate-500 truncate mt-0.5">
            Live Base intelligence · ⭐🟦🦈 · updates every hour
          </p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          {updatedAt > 0 && (
            <span className="font-mono text-[10px] text-slate-600 hidden sm:inline">Last updated {ago(updatedAt)}</span>
          )}
          <button
            onClick={load}
            disabled={loading}
            className="font-mono text-[11px] px-3 py-1.5 rounded-lg border border-[#1A1A2E] text-slate-400 hover:text-white hover:border-[#FB923C]/40 transition-colors disabled:opacity-40"
          >
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-6">
        ...
        {items.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
            {items.map((item) => (
              <FeedCard key={item.id} item={item} copied={copied === item.id} onShare={() => share(item)} />
            ))}
          </div>
        )}
      </div>
