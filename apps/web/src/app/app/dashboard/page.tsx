...
/**
 * /app/dashboard — Unified wallet + stake + alerts overview.
 *
 * Replaces three separate destinations (/app/profile, /app/portfolio,
 * /app/alerts) with a single-glance screen optimised for Base App's
 * mobile-only canvas. Transaction-heavy flows still live at /app/rewards
 * and the alerts CRUD stays at /app/alerts — this page is the read-only
 * landing that links into them.
 *
 * Sections, top to bottom:
 *   1. Identity strip   — wallet, tier, builder score, member-since
 *   2. Earning summary  — staked, daily credits, pending USDC yield, total credits
 *   3. Balances grid    — BLUE / USDC / WETH / cbBTC / AERO
 *   4. Stake position   — read-only summary with deep-link to /app/rewards
 *   5. Active alerts    — last 3 alerts with deep-link to /app/alerts
 *   6. Quick actions    — Stake · Analyze PnL · New alert · Open chat
 *   7. Chat activity    — sessions / messages / credits / top tools
 */
...
// ─── Tier table — kept in sync with /app/profile + /app/rewards ──────────────
...
export default function DashboardPage() {
  ...
  // Single batched read for all on-chain state. wagmi narrows the contracts
  // array as a single tuple type, so heterogeneous ABIs need to be listed
  // inline rather than spread from a .map() — mixed ABIs in a spread lose
  // the per-item type discrimination.
  ...
  return (
    <div className="flex flex-col h-full bg-[#050508] text-white font-mono overflow-hidden">
      ...
      <AppPageHeader
        label="DASHBOARD"
        subtitle="Wallet · holdings · stake · alerts"
        accent={tier.color}
      />
            ...
            <>
              ...
              <AppCard className="mb-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Link href="/app/portfolio" className="text-[10px] text-slate-700 hover:text-[#4FC3F7] transition-colors">
                    Full portfolio →
                  </Link>
                </div>
              ...
              {/* ── 3. Stake position summary + 4. Active alerts ─────────── */}
              ...
              <div className="flex flex-wrap gap-3 text-[10px] text-slate-700 justify-center">
                ...
                <Link href="/app/portfolio" className="hover:text-slate-500 transition-colors">Portfolio →</Link>
                <Link href="/app/rewards" className="hover:text-slate-500 transition-colors">Rewards →</Link>
                <Link href="/app/alerts" className="hover:text-slate-500 transition-colors">Alerts →</Link>
